(function () {
    ...
    angular
        .module('gongrenOps')
        .controller('leaCustomerListController', leaCustomerListController);

    /** @ngInject */
    function leaCustomerListController(notify, $sce, $resource, $compile, $scope, $state, commDict, paginationNum, $stateParams, xxCacheSrv, ModalService) {
        var vm = this;
        //TODO:判断当前是否为superAdmin用户
        var locationName = localStorage.getItem("CURRENT_USER_NAME")
        if (locationName == "李喆") {
            vm.hiddenImport = false;
        }else{
            vm.hiddenImport = true;
        }
        ...
        vm.queryFn = function () {
            ...
            datas.get(vm.query, function (response) {
                if (response.code == 0) {
                    ...
                    // console.log(vm.listDatas, "leaCustomerManage");
                }
            })
        }
        vm._canSelect = 1
        ...
        vm.editSource = function (row) {
            ModalService.showModal({
                templateUrl: "app/modules/leaCustomerManage/editSource.html",
                controller: "leadEditSourceController",
                controllerAs: "modal"
            }).then(function (modal) {
                modal.element.modal();
        ...
        vm.assignPerson = function () {
            ...
            ModalService.showModal({
                templateUrl: "app/modules/leaCustomerManage/assignPerson.html",
                controller: "assignPersonDetailController",
                controllerAs: "modal"
            }).then(function (modal) {
                modal.element.modal();
                modal.close.then(function (result) {
                    if (result) {
                        var datas = $resource("/api/assign/update/accUserAssignByZG");
                        datas.save({
                            userid: result.datas,
                            ids: ids
                        }, function (response) {
        ...
        vm.deleteSource = function () {
            ...
            swal({
                ...
                var datas = $resource("/api/assign/delete/byIds")
                datas.save({
                    ids: ids
                }, function (response) {
                    if (response.code == 0) {
        ...
        vm.importFile = function () {
            ModalService.showModal({
                templateUrl: "app/modules/leaCustomerManage/import.html",
                controller: "leadImportManageController",
                controllerAs: "modal"
            }).then(function (modal) {
                modal.element.modal();
                // $state.reload();
        ...
        vm.todoDetail = function (row) {
            ModalService.showModal({
                templateUrl: "app/modules/leaCustomerManage/todoDetail.html",
